...
0000000000400ee0 <phase_1>:
  400ee0:	48 83 ec 08          	sub    $0x8,%rsp //开栈
  400ee4:	be 00 24 40 00       	mov    $0x402400,%esi //把地址0x402400赋给第二个参数esi，可以通过x/s 0x402400或者x/s $esi查看这个地址存放的字符串
  400ee9:	e8 4a 04 00 00       	callq  401338 <strings_not_equal> //调用一个名叫"strings_not_equal "的函数，如果不等返回1，如果相等返回0
  400eee:	85 c0                	test   %eax,%eax//判断eax是否为0,eax是作为一个返回值参数，因此可以看到这是上一个函数的返回值
  400ef0:	74 05                	je     400ef7 <phase_1+0x17>//相等则跳转语句，下一句就是炸弹，想要绕开炸弹，就要满足跳转语句，即相等
  400ef2:	e8 43 05 00 00       	callq  40143a <explode_bomb>
  400ef7:	48 83 c4 08          	add    $0x8,%rsp //与开栈相对应的栈指针回收
  400efb:	c3                   	retq   

0000000000400efc <phase_2>:
  400efc:	55                   	push   %rbp
  400efd:	53                   	push   %rbx
  400efe:	48 83 ec 28          	sub    $0x28,%rsp //开栈,有40这么大的空间，可以存10个整型变量

  400f02:	48 89 e6             	mov    %rsp,%rsi //%rsi=%rsp
  400f05:	e8 52 05 00 00       	callq  40145c <read_six_numbers>//调用一个叫“read_six_numbers”的函数
  400f0a:	83 3c 24 01          	cmpl   $0x1,(%rsp)//比较%rsp这个地址管理的内容与立即数0x1的大小
  400f0e:	74 20                	je     400f30 <phase_2+0x34>//相等跳转语句，下一句炸弹提示我们要满足相等条件，相等则跳转到400f30的位置，在375行. 这说明rsp管理的内容就是1。用gdb调试时可以先把断点打在这里，即b *0x400f0e
  400f10:	e8 25 05 00 00       	callq  40143a <explode_bomb>
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>
  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax //让eax指向第5个数的位置,eax=rbx-0x4；第二轮，让rbx指向第3个数的位置
  400f1a:	01 c0                	add    %eax,%eax //eax=eax+eax
  400f1c:	39 03                	cmp    %eax,(%rbx) //比较rbx存放的数和eax
  400f1e:	74 05                	je     400f25 <phase_2+0x29> //相等则跳转语句，由下一句炸弹可知，必须相等，因此eax=rbx
  400f20:	e8 15 05 00 00       	callq  40143a <explode_bomb>
  400f25:	48 83 c3 04          	add    $0x4,%rbx //rbx=rbx+0x4让rbx指向第二个数的位置
  400f29:	48 39 eb             	cmp    %rbp,%rbx //让现在在第二个数的rbx与指向第6个数位置的rbp相比较
  400f2c:	75 e9                	jne    400f17 <phase_2+0x1b>//不相等则跳转语句，跳转至366行
  400f2e:	eb 0c                	jmp    400f3c <phase_2+0x40>//否则相等就跳转到栈指针回收的位置
  400f30:	48 8d 5c 24 04       	lea    0x4(%rsp),%rbx//让rbx指向rsp+0x4的位置，中间4字节刚好是一个整型变量的大小，可以认为这是6个数中的第一个数的位置,同时rbx是被调用者保存寄存器，因此可以认为rbx管理的值就是栈顶元素1
  400f35:	48 8d 6c 24 18       	lea    0x18(%rsp),%rbp//让rbp指向rsp+0x18的位置，中间24字节刚好是6个整型变量的大小，可以认为这是6个数中最后一个数的位置
  400f3a:	eb db                	jmp    400f17 <phase_2+0x1b>//无条件跳转到400f17，即366行
...
0000000000400f43 <phase_3>:
  400f43:	48 83 ec 18          	sub    $0x18,%rsp//开栈，24个字节
  400f47:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx//rcx指向栈中间
  400f4c:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx//rdx指向rsp+8的位置
  400f51:	be cf 25 40 00       	mov    $0x4025cf,%esi//第一个参数esi=0x4025cf
  400f56:	b8 00 00 00 00       	mov    $0x0,%eax
  400f5b:	e8 90 fc ff ff       	callq  400bf0 <__isoc99_sscanf@plt>
  400f60:	83 f8 01             	cmp    $0x1,%eax
  400f63:	7f 05                	jg     400f6a <phase_3+0x27>//大于跳转，上一句和1比较，说明我们要输入2个参数
  400f65:	e8 d0 04 00 00       	callq  40143a <explode_bomb>
  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp)//第3个参数和7比较
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a>//如果超过了，就会跳到炸弹，所以不能超过
  400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax//eax指向rsp+8，也不必纠结地址大小，只要明白它说的是第2块空间，即输入的第1个参数
  400f75:	ff 24 c5 70 24 40 00 	jmpq   *0x402470(,%rax,8)//将有8个case的跳转表，rax只是一个计数器，case值都小于或等于7,
  //通过x/8xg 0x402470可以查看跳转表的地址，比如如果是0，就跳转到0x0000000000400f7c的地址，如果是1，就跳转到0x0000000000400fb9
  ...
  400fbe:	3b 44 24 0c          	cmp    0xc(%rsp),%eax//不用纠结地址比较，只要清楚它说的是第3块空间，即输入的第2个参数
...
0000000000400fce <func4>:
  400fce:	48 83 ec 08          	sub    $0x8,%rsp//开栈2块4字节的空间
  400fd2:	89 d0                	mov    %edx,%eax//eax存储第3个参数，即eax=14
  400fd4:	29 f0                	sub    %esi,%eax//eax=eax-esi=14-0=14
  400fd6:	89 c1                	mov    %eax,%ecx//ecx=14
  400fd8:	c1 e9 1f             	shr    $0x1f,%ecx//第4个参数右移31位，即取最高位,ecx=0
  400fdb:	01 c8                	add    %ecx,%eax//eax存储最高位，eax=14+0=14
  400fdd:	d1 f8                	sar    %eax//eax=14>>1=7
  400fdf:	8d 0c 30             	lea    (%rax,%rsi,1),%ecx//ecx=rax+rsi*1=7+0*1=7
  400fe2:	39 f9                	cmp    %edi,%ecx//比较7和第1个参数edi的大小
  400fe4:	7e 0c                	jle    400ff2 <func4+0x24>//小于等于跳转语句
  400fe6:	8d 51 ff             	lea    -0x1(%rcx),%edx//第一个数小于7，就跳转此处，edx=7-1=6
  400fe9:	e8 e0 ff ff ff       	callq  400fce <func4>//又回到函数，因此函数的判断条件是小于7
  400fee:	01 c0                	add    %eax,%eax
  400ff0:	eb 15                	jmp    401007 <func4+0x39>
  400ff2:	b8 00 00 00 00       	mov    $0x0,%eax//第一个参数edi大于等于7跳转此处，且eax=0
  400ff7:	39 f9                	cmp    %edi,%ecx//比较7和第1个参数edi的大小
  400ff9:	7d 0c                	jge    401007 <func4+0x39>大于等于跳转，跳转后栈空间回收
  400ffb:	8d 71 01             	lea    0x1(%rcx),%esi//如果ecx<edi,则esi=ecx+1
...
000000000040100c <phase_4>:
  40100c:	48 83 ec 18          	sub    $0x18,%rsp//开栈，24个字节，6块4字节的空间
  401010:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx
  401015:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx
  40101a:	be cf 25 40 00       	mov    $0x4025cf,%esi//0x4025cf是包含两个十进制数的字符串，esi是第二个参数
  40101f:	b8 00 00 00 00       	mov    $0x0,%eax//eax=0
  401024:	e8 c7 fb ff ff       	callq  400bf0 <__isoc99_sscanf@plt>
  401029:	83 f8 02             	cmp    $0x2,%eax
  40102c:	75 07                	jne    401035 <phase_4+0x29>//如果不相等就炸掉，所以必须相等，因此eax必须输入2个参数
  40102e:	83 7c 24 08 0e       	cmpl   $0xe,0x8(%rsp)//rsp移动到从栈顶开始算的第3快空间的起始位置
  401033:	76 05                	jbe    40103a <phase_4+0x2e>小于等于跳转语句，下一行就是炸弹，所以必须满足小于或者等于，即第1个数要小于等于14
  401035:	e8 00 04 00 00       	callq  40143a <explode_bomb>
  40103a:	ba 0e 00 00 00       	mov    $0xe,%edx//edx=0xe=14第3个参数
  40103f:	be 00 00 00 00       	mov    $0x0,%esi//esi=0第2个参数
  401044:	8b 7c 24 08          	mov    0x8(%rsp),%edi//第1个参数edi指向栈顶往上的第3块空间的起始位置
  401048:	e8 81 ff ff ff       	callq  400fce <func4>//调用400fce地址的函数
  40104d:	85 c0                	test   %eax,%eax//检测返回值，下一句如果不为0就爆炸，所以eax返回0
  40104f:	75 07                	jne    401058 <phase_4+0x4c>
  401051:	83 7c 24 0c 00       	cmpl   $0x0,0xc(%rsp)//比较栈顶往上数的第4个空间存储的值和0的大小
  401056:	74 05                	je     40105d <phase_4+0x51>等于跳转，下一句是炸弹，所以必须满足等于，因此第4个空间存储的值就是0
  401058:	e8 dd 03 00 00       	callq  40143a <explode_bomb>
  40105d:	48 83 c4 18          	add    $0x18,%rsp//栈空间回收
  401061:	c3                   	retq   

0000000000401062 <phase_5>:
  401062:	53                   	push   %rbx
  401063:	48 83 ec 20          	sub    $0x20,%rsp//开栈32字节的空间
  401067:	48 89 fb             	mov    %rdi,%rbx//rbx=rdi，即字符串起始地址保存在rbx中
  40106a:	64 48 8b 04 25 28 00 	mov    %fs:0x28,%rax//栈破坏检测，金丝雀值，在程序恢复寄存器从函数返回之前，检查金丝雀的值是否被改变，如果是，那么程序异常终止
  401071:	00 00 
  401073:	48 89 44 24 18       	mov    %rax,0x18(%rsp)//rsp指向第7个空间起始位置，并且值为rax
  401078:	31 c0                	xor    %eax,%eax//对同一个数异或操作即对eax清0处理，用作后面的循环计数器
  40107a:	e8 9c 02 00 00       	callq  40131b <string_length>//调用string_length函数,计算字符串的长度，将长度存储在eax中
  40107f:	83 f8 06             	cmp    $0x6,%eax//比较eax和6的大小
  401082:	74 4e                	je     4010d2 <phase_5+0x70>//相等跳转语句，由下一句炸弹可知必须相等，因此eax=6，因此字符串长度为6才合法
  401084:	e8 b1 03 00 00       	callq  40143a <explode_bomb>
  401089:	eb 47                	jmp    4010d2 <phase_5+0x70>//跳转至504行
  40108b:	0f b6 0c 03          	movzbl (%rbx,%rax,1),%ecx//ecx=edi+eax, 即ecx从第一个字符遍历到第6个字符,把输入字符串的第eax个字符赋值给ecx
  40108f:	88 0c 24             	mov    %cl,(%rsp)//取指令rcx的低8位,(%rsp)=rcx的低8位
  401092:	48 8b 14 24          	mov    (%rsp),%rdx//rdx=c1=rcx的低8位
  401096:	83 e2 0f             	and    $0xf,%edx//取rdx的低4位,即取rcx的低4位
  401099:	0f b6 92 b0 24 40 00 	movzbl 0x4024b0(%rdx),%edx//edx=0x4024b0+edx=0x4024b0+edx低4位,由于rdx只有4位有效，最多表示16个不同位置，查看0x4024b0的内容：x/s 0x4024b0，即 "maduiersnfotvbylSo you think you can stop the bomb with ctrl-c, do you?"，所以0x4024b0+edx映射到maduiersnfotvbyl
  4010a0:	88 54 04 10          	mov    %dl,0x10(%rsp,%rax,1)//dl为0x4024b0+edx表示的字符，把它赋值给rax+rsp+0x10，输出的字符串后4位的ASC码的会对应上面的字符串
  4010a4:	48 83 c0 01          	add    $0x1,%rax//rax=rax+1,计数器加1
  4010a8:	48 83 f8 06          	cmp    $0x6,%rax//比较rax和6的大小
  4010ac:	75 dd                	jne    40108b <phase_5+0x29>//不等跳转语句，继续循环
  4010ae:	c6 44 24 16 00       	movb   $0x0,0x16(%rsp)//等于跳转，rsp+0x16=0，实际上就是给字符串加尾0
  4010b3:	be 5e 24 40 00       	mov    $0x40245e,%esi//第2个参数esi=0x40245e,用x/s 0x40245e可知它是"flyers"，依次对应maduiersnfotvbyl的第9、F、E、5、6、7（从0开始数）个，符号条件的字符串为9ON567
  4010b8:	48 8d 7c 24 10       	lea    0x10(%rsp),%rdi//继续让rsp偏移16bite，指向rdi
  4010bd:	e8 76 02 00 00       	callq  401338 <strings_not_equal>//比较字符串是否相等
  4010c2:	85 c0                	test   %eax,%eax//检测返回值是否为0
  4010c4:	74 13                	je     4010d9 <phase_5+0x77>相等跳转，下一条炸弹告诉我们必须相等
  4010c6:	e8 6f 03 00 00       	callq  40143a <explode_bomb>
  4010cb:	0f 1f 44 00 00       	nopl   0x0(%rax,%rax,1)//rax=rax+1
  4010d0:	eb 07                	jmp    4010d9 <phase_5+0x77>
  4010d2:	b8 00 00 00 00       	mov    $0x0,%eax//eax=0
  4010d7:	eb b2                	jmp    40108b <phase_5+0x29>//跳转486行
  4010d9:	48 8b 44 24 18       	mov    0x18(%rsp),%rax//rax=rsp+24
  4010de:	64 48 33 04 25 28 00 	xor    %fs:0x28,%rax//金丝雀检测
  4010e5:	00 00 
  4010e7:	74 05                	je     4010ee <phase_5+0x8c>
  4010e9:	e8 42 fa ff ff       	callq  400b30 <__stack_chk_fail@plt>
  4010ee:	48 83 c4 20          	add    $0x20,%rsp//出栈
  4010f2:	5b                   	pop    %rbx
  4010f3:	c3                   	retq   
